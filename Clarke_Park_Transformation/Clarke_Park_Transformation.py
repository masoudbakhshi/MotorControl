...
# ---------------------------
# OUTPUT PATHS (auto-create)
# ---------------------------
OUT_DIR = os.path.join("FOC_Tutorial", "Visuals", "Animations")
os.makedirs(OUT_DIR, exist_ok=True)
OUT_MP4 = os.path.join(OUT_DIR, "01_ClarkePark.mp4")
# Optional: also keep a lightweight GIF for quick previews
OUT_GIF = os.path.join(OUT_DIR, "01_ClarkePark_preview.gif")
...
FPS = 25
DPI = 120
FIGSIZE = (16, 9)   # 16*120 by 9*120 => 1920x1080
# H.264 settings: visually lossless-ish but compact
FFMPEG_ARGS = [
    "-vcodec", "libx264",
    "-pix_fmt", "yuv420p",
    "-crf", "18",          # lower = higher quality (typical 17–20)
    "-preset", "slow",     # slow = better compression at same quality
    "-movflags", "+faststart"  # better streaming behavior
]

# ---------------------------
# ANIMATION PARAMETERS
# ---------------------------
omega_elec = 2 * np.pi / 5    # rad/s (period = 5 s)
frames = 200                   # @25 fps -> ~8.0 s runtime
t_max = 2 * np.pi / omega_elec # one electrical cycle
t = np.linspace(0, t_max, frames)

# Three-phase balanced currents
...
fig.suptitle(
    "ABC to dq Transformation: Clarke, Stator-Fixed dq, and Synchronous dq Frames",
    fontsize=18, y=0.97
)
# Branding (kept per your requirement)
...
for angle, col, label in zip(phase_angle, phase_colors, phase_labels):
    axC.arrow(0, 0, np.cos(angle), np.sin(angle), width=0.01, head_width=0.06,
              color=col, length_includes_head=True, alpha=0.8)
...
# === dq plots (stator-fixed and synchronous) ===
axes_dq = []
dq_labels = []
for col, (theta_arr, idq, title) in enumerate([
    ...
    # placeholders updated every frame
    ...
    dq_labels.append((
        ax.text(0, 0, "", fontsize=11, color="#90ee90", weight="bold"),  # d label
...
def animate(i):
    ...
    if axC.clarke_vector is not None:
        axC.clarke_vector.remove()
    axC.clarke_vector = axC.arrow(0, 0, i_alpha[i], i_beta[i],
                                  color="magenta", width=0.014, head_width=0.08,
    ...
    for idx, (ax, theta_arr, idq) in enumerate(zip(axes_dq, [theta_fixed, theta_sync], [idq_fixed, idq_sync])):
        ...
        # Clarke vector
        ax.vec_clarke = ax.arrow(0, 0, i_alpha[i], i_beta[i], color="magenta",
...
# --- Save MP4 (primary) ---
try:
    writer = FFMpegWriter(fps=FPS, metadata={"title": "ABC→dq Transformation", "artist": "Masoud Bakhshi"},
                          extra_args=FFMPEG_ARGS)
    ani.save(OUT_MP4, writer=writer, dpi=DPI)
    print(f"[OK] Saved MP4: {OUT_MP4}")
except FileNotFoundError as e:
    print("[ERROR] FFmpeg not found. Install it via pip/conda. Falling back to GIF preview…")
    ani.save(OUT_GIF, writer="pillow", fps=FPS, dpi=DPI)
    print(f"[OK] Saved GIF preview: {OUT_GIF}")
